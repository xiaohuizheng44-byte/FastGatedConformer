...
def dataset_to_file(dataset_name, data_save):
    if moabb is None:
        raise ImportError('moabb is required only for dataset_to_file(). Install moabb or put processed .npy files under ./data/.')
...
def data_loader(Xs=None, Ys=None, Xt=None, Yt=None, args=None):
    ...
    source_indices = tr.arange(Xs.shape[0]).long()
    if args.data_env != 'local':
        Xs, Ys, Xt, Yt, Xt_copy, source_indices = Xs.cuda(), Ys.cuda(), Xt.cuda(), Yt.cuda(), Xt_copy.cuda(), source_indices.cuda()

    if getattr(args, 'return_source_index', False):
        data_src = Data.TensorDataset(Xs, Ys, source_indices)
    else:
